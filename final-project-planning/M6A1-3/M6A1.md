...
The finished scope keeps the three core routes above and adds the Point of Sale Quick Cart within `/inventory`, a receipt confirmation, sales analytics on `/`, product-image support, and an owner sign-in gate for Live Mode. These are contained features rather than extra top-level routes, so the primary navigation stays simple for a store owner.
